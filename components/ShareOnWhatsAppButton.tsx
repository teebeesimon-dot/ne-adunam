...
export default function ShareOnWhatsAppButton({
  ...
  useEffect(() => {
    setShareUrl(buildWhatsAppShareUrl(event, window.location.href));
  }, [event]);
  ...
  return (
    <a
      ...
      className={`inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#25D366] bg-[#25D366] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#20BD5A] active:scale-[0.98] sm:w-auto ${className}`}
    >
      <WhatsAppIcon />
      Share on WhatsApp
    </a>
